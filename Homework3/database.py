...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    if _is_sqlite(DATABASE_URL):
        # SQLite has no row locking; a writer reservation serializes claims.
        # PostgreSQL instead uses FOR UPDATE SKIP LOCKED on the claim query
        # (see storage.claim_one), so a plain transaction is enough here.
