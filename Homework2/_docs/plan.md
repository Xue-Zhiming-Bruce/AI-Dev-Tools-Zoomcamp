...
## Stack direction (per homework requirements)

- Frontend: Node.js-based SPA in `frontend/`, backend calls centralized and mocked first
- Backend: FastAPI in `backend/`, managed with `uv`; mock DB first, then SQLAlchemy (SQLite)
- Contract: OpenAPI as source of truth between frontend and backend
